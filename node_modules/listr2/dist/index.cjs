...
__name(BaseEventMap, "BaseEventMap");
...
var ListrLogger = class {
  ...
  format(level, message, options) {
    ...
    message = this.splat(message.shift(), ...message).split(import_os.EOL).filter((m) => !m || m.trim() !== "").map((m) => {
      return this.style(
        level,
        this.fields(m, {
...
__name(ProcessOutputBuffer, "ProcessOutputBuffer");
...
__name(TestRendererSerializer, "TestRendererSerializer");
...
__name(ListrRendererError, "ListrRendererError");
...
__name(ListrTaskEventManager, "ListrTaskEventManager");
...
__name(ListrEventManager, "ListrEventManager");
