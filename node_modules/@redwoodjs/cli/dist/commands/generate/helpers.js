...
exports.yargsDefaults = exports.validateName = exports.templateForComponentFile = void 0;
...
const customOrDefaultTemplatePath = ({
  ...
  const defaultPath = _path.default.join(__dirname, generator, 'templates', templatePath);
...
const templateForComponentFile = ({
  ...
  const content = (0, _lib.generateTemplate)(fullTemplatePath, {
    name,
    outputPath: (0, _projectConfig.ensurePosixPath)(`./${_path.default.relative((0, _lib.getPaths)().base, componentOutputPath)}`),
    ...templateVars
  });
...
const appendPositionalsToCmd = (commandString, positionalsObj) => {
  ...
  if ((0, _keys.default)(positionalsObj).length > 0) {
    var _context;
    const positionalNames = (0, _map.default)(_context = (0, _keys.default)(positionalsObj)).call(_context, positionalName => `[${positionalName}]`).join(' ');
...
function removeGeneratorName(name, generatorName) {
  // page -> Page
  const pascalComponentName = (0, _pascalcase.default)(generatorName);
  ...
  return {
    ...
    handler: async options => {
      if (options.tests === undefined) {
        options.tests = (0, _projectConfig.getConfig)().generate.tests;
      }
      if (options.stories === undefined) {
        options.stories = (0, _projectConfig.getConfig)().generate.stories;
      }
