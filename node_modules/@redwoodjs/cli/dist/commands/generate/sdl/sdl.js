...
const modelFieldToSDL = ({
  ...
  const fieldContent = `${field.name}: ${field.isList ? '[' : ''}${dictionary[field.type] || field.type}${field.isList ? ']' : ''}${(field.isRequired && required) | field.isList ? '!' : ''}`;
  if (docs) {
...
const handler = async ({
  ...
  try {
    ...
    const tasks = new _listr.Listr((0, _filter.default)(_context12 = [{
