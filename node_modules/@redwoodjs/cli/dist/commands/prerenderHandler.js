...
const getTasks = async (dryrun, routerPathFilter = null) => {
  ...
  const indexHtmlPath = _path.default.join((0, _projectConfig.getPaths)().web.dist, 'index.html');
    ...
    return [{
      ...
      task: async () => {
        ...
        if (/\{.*}/.test(routeToPrerender.path)) {
          throw new PathParamError(`Could not retrieve route parameters for ${routeToPrerender.path}`);
        }
        try {
...
const diagnosticCheck = () => {
  const checks = [{
    ...
    failure: _fs.default.existsSync(_path.default.join((0, _projectConfig.getPaths)().web.base, 'node_modules/react'))
  }, {
    message: 'Duplicate react-dom version found in web/node_modules',
    failure: _fs.default.existsSync(_path.default.join((0, _projectConfig.getPaths)().web.base, 'node_modules/react-dom'))
  }, {
    message: 'Duplicate core-js version found in web/node_modules',
    failure: _fs.default.existsSync(_path.default.join((0, _projectConfig.getPaths)().web.base, 'node_modules/core-js'))
  }, {
    message: 'Duplicate @redwoodjs/web version found in web/node_modules',
    failure: _fs.default.existsSync(_path.default.join((0, _projectConfig.getPaths)().web.base, 'node_modules/@redwoodjs/web'))
  }];
...
const handler = async ({
  ...
  const tasks = new _listr.Listr(listrTasks, {
  ...
  try {
    ...
    if (e instanceof PathParamError) {
      ...
      console.log(_colors.default.info('- Avoid prerendering Cells with authenticated queries, by conditionally rendering them.\n  See https://redwoodjs.com/docs/prerender#common-warnings--errors'));
    }
    console.log();
    process.exit(1);
  }
