...
const pathJoin = _path.default.posix.join;
const execaOptions = {
  cwd: pathJoin((0, _lib.getPaths)().base),
  stdio: 'inherit',
  shell: true,
  cleanup: true
};
...
const maintenanceTasks = (status, ssh, serverConfig) => {
  ...
  if (status === 'up') {
    tasks.push({
      ...
      task: async () => {
        await sshExec(ssh, deployPath, 'cp', [pathJoin('web', 'dist', '200.html'), pathJoin('web', 'dist', '200.html.orig')]);
        await sshExec(ssh, deployPath, 'ln', [SYMLINK_FLAGS, pathJoin('..', 'src', 'maintenance.html'), pathJoin('web', 'dist', '200.html')]);
      }
    });
    ...
    if (serverConfig.processNames) {
      tasks.push({
        ...
        task: async () => {
          await sshExec(ssh, deployPath, 'rm', [pathJoin('web', 'dist', '200.html')]);
          await sshExec(ssh, deployPath, 'cp', [pathJoin('web', 'dist', '200.html.orig'), pathJoin('web', 'dist', '200.html')]);
        }
      });
    }
  }
      ...
      if (yargs.firstRun) {
        tasks.push(commandWithLifecycleEvents({
          ...
          command: {
            ...
            task: async () => {
              await sshExec(ssh, serverConfig.path, serverConfig.monitorCommand, ['start', pathJoin(CURRENT_RELEASE_SYMLINK_NAME, 'ecosystem.config.js'), '--only', processName]);
            }
          }
        }));
        tasks.push({
...
const parseConfig = (yargs, rawConfigToml) => {
  const configToml = (0, _stringEnvInterpolation.env)(rawConfigToml);
...
const commands = (yargs, ssh) => {
  const deployConfig = _fs.default.readFileSync(pathJoin((0, _lib.getPaths)().base, CONFIG_FILENAME)).toString();
  let {
    envConfig,
    envLifecycle
...
const handler = async yargs => {
  ...
  try {
    const tasks = new _listr.Listr(commands(yargs, ssh), {
      concurrent: true,
      exitOnError: true,
