...
const getFlightcontrolJson = async database => {
  ...
  const schema = await (0, _internals.getSchema)(_path.default.join((0, _lib.getPaths)().base, 'api/db/schema.prisma'));
  const config = await (0, _internals.getConfig)({
    datamodel: schema
  });
  ...
  if (detectedDatabase === database) {
    ...
    return {
      path: _path.default.join((0, _lib.getPaths)().base, 'flightcontrol.json'),
      content: {
        ..._flightcontrol.flightcontrolConfig,
        environments: [{
          ..._flightcontrol.flightcontrolConfig.environments[0],
