...
const handler = async ({
  ...
  if (apiPortChangeNeeded || webPortChangeNeeded) {
    ...
    console.error(`Cannot run the development server until your configured ports are changed or become available.`);
    process.exit(1);
  }
  if ((0, _includes.default)(side).call(side, 'api')) {
    try {
  ...
  const webCommand = (0, _projectConfig.getConfig)().web.bundler === 'vite' // @NOTE: can't use enums, not TS
  ? `yarn cross-env NODE_ENV=development rw-vite-dev` : `yarn cross-env NODE_ENV=development RWJS_WATCH_NODE_MODULES=${watchNodeModules ? '1' : ''} webpack serve --config "${webpackDevConfig}" ${forward}`;
