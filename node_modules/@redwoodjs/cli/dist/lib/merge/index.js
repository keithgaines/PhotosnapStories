...
function mergeAST(baseAST, extAST, strategy = {}) {
  ...
  const [imports, others] = _lodash._.partition(getProgramPath(extAST).get('body'), (0, _algorithms.nodeIs)('ImportDeclaration'));
  (0, _forEach.default)(imports).call(imports, exp => insertAfterLastImport(exp, baseProgram));
  _lodash._.forEachRight(others, exp => insertBeforeFirstUsage(exp, baseProgram));
}
...
function merge(base, extension, strategy) {
  ...
  return process.env.JEST_WORKER_ID ? _prettier.default.format(code, {
    parser: 'babel',
    bracketSpacing: true,
    tabWidth: 2,
    semi: false,
