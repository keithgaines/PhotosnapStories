...
async function runScriptFunction({
  ...
  const script = await (specifier => new _promise.default(r => r(`${specifier}`)).then(s => (0, _interopRequireWildcard2.default)(require(s))))(scriptPath);
  const returnValue = await script[functionName](args);
  try {
    const {
      db
    } = await (specifier => new _promise.default(r => r(`${specifier}`)).then(s => (0, _interopRequireWildcard2.default)(require(s))))(_path.default.join((0, _projectConfig.getPaths)().api.lib, 'db'));
    db.$disconnect();
  } catch (e) {
    // silence
  }
...
async function configureBabel() {
  const {
    ...
    plugins: webPlugins,
    ...otherWebConfig
  } = (0, _web.getWebSideDefaultBabelConfig)();
