...
function fileIncludes(path, str) {
  var _context;
  return _fs.default.existsSync(path) && (0, _includes.default)(_context = _fs.default.readFileSync(path).toString()).call(_context, str);
}
...
function extendJSXFile(path, {
  ...
  const content = _fs.default.readFileSync(path).toString().split('\n');
  ...
  _fs.default.writeFileSync(path, (0, _filter.default)(content).call(content, e => e !== undefined).join('\n'));
}
