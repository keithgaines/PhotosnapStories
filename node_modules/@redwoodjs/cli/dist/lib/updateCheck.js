...
exports.LOCK_IDENTIFIER = exports.EXCLUDED_COMMANDS = exports.DEFAULT_DATETIME_MS = void 0;
exports.check = check;
 ...
 */
const DEFAULT_DATETIME_MS = 946684800000;

/**
 * @const {string} The identifier used for the lock within the check function
 */
exports.DEFAULT_DATETIME_MS = DEFAULT_DATETIME_MS;
const LOCK_IDENTIFIER = 'UPDATE_CHECK';

/**
 * @const {string[]} The name of commands which should NOT execute the update checker
 */
exports.LOCK_IDENTIFIER = LOCK_IDENTIFIER;
 ...
 */
exports.EXCLUDED_COMMANDS = EXCLUDED_COMMANDS;
...
async function check() {
  try {
    (0, _locking.setLock)(LOCK_IDENTIFIER);

    // Read package.json and extract the @redwood/core version
    const packageJson = JSON.parse(_fs.default.readFileSync(_path.default.join((0, _index.getPaths)().base, 'package.json')));
    ...
    (0, _locking.unsetLock)(LOCK_IDENTIFIER);
  }
}
...
function readUpdateDataFile() {
  try {
    if (!_fs.default.existsSync(getPersistenceDirectory())) {
      _fs.default.mkdirSync(getPersistenceDirectory());
    }
    const persistedData = JSON.parse(_fs.default.readFileSync(_path.default.join(getPersistenceDirectory(), 'data.json')));
    // Reconstruct the map
    persistedData.remoteVersions = new _map.default((0, _entries.default)(persistedData.remoteVersions));
...
function updateUpdateDataFile({
  ...
  _fs.default.writeFileSync(_path.default.join(getPersistenceDirectory(), 'data.json'), (0, _stringify.default)(updatedData, null, 2));
}
...
function updateCheckMiddleware(argv) {
  ...
  if (shouldCheck()) {
    ...
    const stdout = _fs.default.openSync(_path.default.join(getPersistenceDirectory(), 'stdout.log'), 'w');
    const stderr = _fs.default.openSync(_path.default.join(getPersistenceDirectory(), 'stderr.log'), 'w');
    ...
    const child = (0, _child_process.spawn)('yarn', ['node', _path.default.join(__dirname, 'updateCheckExecute.js')], spawnOptions);
    child.unref();
  }
}