...
function addFileToRollback(path, atEnd = false) {
  const step = {
    type: 'file',
    path: path,
    content: _fs.default.existsSync(path) ? _fs.default.readFileSync(path) : null
  };
  if (atEnd) {
    rollback.unshift(step);
  } else {
    ...
    switch (step.type) {
      ...
      case 'file':
        if (step.content === null) {
          _fs.default.unlinkSync(step.path);
          ...
          if (parent !== '.' && _fs.default.readdirSync(parent).length === 0) {
            _fs.default.rmdirSync(parent);
          }
          ...
          _fs.default.writeFileSync(step.path, step.content);
        }
        break;
      default:
