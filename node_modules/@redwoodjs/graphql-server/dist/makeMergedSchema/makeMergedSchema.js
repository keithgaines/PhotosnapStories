...
const wrapWithOpenTelemetry = async (func, args, root, context, info, name) => {
  ...
  return await tracer.startActiveSpan(`redwoodjs:graphql:resolver:${name}`, {}, parentContext, async span => {
    span.setAttribute('graphql.execute.operationName', `${args.operationName || 'Anonymous Operation'}`);
    try {
    ...
    if (services?.[name]) {
      return {
        ...
        [name]: async (root, args, context, info) => {
          ...
          try {
            experimentalOpenTelemetryEnabled = (0, _projectConfig.getConfig)().experimental.opentelemetry.enabled;
          } catch (e) {
            // Swallow the error for now
          }
          if (experimentalOpenTelemetryEnabled) {
            return wrapWithOpenTelemetry(services[name], args, root, context, info, name);
          }
...
const mergeResolversWithServices = ({
  ...
  const mergedServices = (0, _lodash.default)({}, ...(0, _map.default)(_context4 = (0, _keys.default)(services)).call(_context4, name => services[name]));
  ...
  })).call(_context5, type => type !== undefined && type !== null);
...
const makeMergedSchema = ({
  ...
  const typeDefs = mergeTypes([rootGqlSchema.schema, ...(0, _map.default)(directives).call(directives, directive => directive.schema),
