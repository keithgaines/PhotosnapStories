...
const rebuildApiServer = () => {
  try {
    ...
    if ((0, _projectConfig.getConfig)().experimental.opentelemetry.enabled) {
      const opentelemetrySDKScriptPath = (0, _projectConfig.getConfig)().experimental.opentelemetry.apiSdk;
      if (opentelemetrySDKScriptPath) {
        console.log(`Setting up OpenTelemetry using the setup file: ${opentelemetrySDKScriptPath}`);
        if (_fs.default.existsSync(opentelemetrySDKScriptPath)) {
          ...
          console.error(`OpenTelemetry setup file does not exist at ${opentelemetrySDKScriptPath}`);
        }
      }
    }
...
const delayRestartServer = (0, _lodash.debounce)(rebuildApiServer, process.env.RWJS_DELAY_RESTART ? (0, _parseInt2.default)(process.env.RWJS_DELAY_RESTART, 10) : 5);
