...
const bothServerHandler = async options => {
  ...
  const apiRootPath = coerceRootPath((0, _projectConfig.getConfig)().web.apiUrl);
  let fastify = (0, _fastify.default)();
...
const webServerHandler = async options => {
  ...
  const apiUrl = (0, _projectConfig.getConfig)().web.apiUrl;
  // Construct the graphql url from apiUrl by default
  // But if apiGraphQLUrl is specified, use that instead
  const graphqlEndpoint = coerceRootPath((0, _projectConfig.getConfig)().web.apiGraphQLUrl ?? `${apiUrl}/graphql`);
  let fastify = (0, _fastify.default)();
